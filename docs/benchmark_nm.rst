...
The script enforces a fair setup:

* same analytic objective function for every run,
* same start points ``x0``,
* same initial simplex geometry (``x0 + step * e_i``),
* same Nelder-Mead coefficients (``alpha=1``, ``gamma=2``, contractions
  ``0.5``, ``sigma=0.5``),
* same stopping budgets (``max_iter``, ``max_evals``),
* method-specific extras disabled (``adaptive=False``, ``memoize=False``,
  ``normalize=False``, ``parallel_poll_points=False``).
...
Configuration is intentionally fixed in the script via module-level constants:
